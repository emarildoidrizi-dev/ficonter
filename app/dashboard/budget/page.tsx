...
export default async function BudgetPage() {
  ...
  const [transactionResult, billResult, planResult, itemResult, goalResult] = await Promise.all([
    supabase.from("transactions").select("id,user_id,description,amount,currency,amount_eur,type,category,transaction_date,occurred_at,exchange_rate_source").eq("user_id", user.id).order("occurred_at", { ascending: false }),
  ...
  return <MonthlyPlanner userId={user.id} initialTransactions={transactionResult.data ?? []} initialBills={billResult.data ?? []} initialPlans={planResult.data ?? []} initialItems={itemResult.data ?? []} initialGoals={goalResult.data ?? []} initialError={plannerError} showAdvancedPosition={showAdvancedPosition} />;
}
