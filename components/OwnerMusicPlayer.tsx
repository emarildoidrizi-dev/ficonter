...
function playbackErrorMessage(reason: unknown): string {
  if (reason instanceof DOMException && reason.name === "NotAllowedError") {
    return "Playback was blocked by the browser. Press Play again to allow audio.";
  }
  if (reason instanceof DOMException && reason.name === "NotSupportedError") {
    return "This track could not be played by this browser. MP3 or AAC is recommended.";
  }
  return reason instanceof Error
    ? reason.message || "The track could not be played."
...
export function OwnerMusicPlayer() {
  ...
  async function startSelectedTrack(trackId: string) {
    setError("");
    try {
      await selectAndPlay(trackId);
    } catch (reason) {
      setError(playbackErrorMessage(reason));
    }
  }
        ...
        <div className={styles.panel}>
          ...
          <div className={styles.transport}>
            ...
            <button
              ...
              onClick={() => player.playing ? pause() : void startCurrentTrack()}
                  ...
                  {player.tracks.map((track, index) => {
                    ...
                    return (
                      <div key={track.id} className={`${styles.trackRow}${active ? ` ${styles.trackRowActive}` : ""}`}>
                        <button type="button" className={styles.trackPlay} onClick={() => active && player.playing ? pause() : void startSelectedTrack(track.id)} aria-label={`${active && player.playing ? "Pause" : "Play"} ${track.title}`}>
