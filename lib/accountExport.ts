...
export async function createAccountPdf(payload: AccountExportPayload): Promise<Blob> {
  ...
  report.table(
    ...
    [
      { label: "Month", width: 190 },
      { label: "Starting balance", width: 230, align: "right" },
      { label: "Monthly budget", width: 230, align: "right" },
      { label: "Created", width: 220 },
    ...
    payload.data.monthly_budget_plans.map((item) => [
      ...
      currency(valueOf(item, "spending_budget"), preferredCurrency, locale),
