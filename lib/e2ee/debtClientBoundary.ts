...
function debtPrivatePayload(record: RecordValue): DebtPrivatePayloadV1 {
  return {
...
function stripPrivateFields(record: RecordValue): RecordValue {
  const sanitized: RecordValue = { ...record, debt_kind: "standard" };
  for (const field of PRIVATE_FIELDS) sanitized[field] = null;
  return sanitized;
}

function restorePrivateFields(row: unknown, original: RecordValue): unknown {
  if (!row || typeof row !== "object" || Array.isArray(row)) return row;
  const restored: RecordValue = { ...(row as RecordValue) };
...
function restoreMutationResult(result: any, original: RecordValue): any {
  if (!result || typeof result !== "object") return result;
  if (Array.isArray(result.data)) {
    return {
      ...result,
      data: result.data.map((row: unknown) => restorePrivateFields(row, original)),
    };
  }
  if (result.data && typeof result.data === "object") {
    return { ...result, data: restorePrivateFields(result.data, original) };
  }
  return result;
}

function deferredMutation(execute: (calls: DeferredCall[]) => Promise<unknown>): any {
...
export function installDebtE2eeBoundary(client: any, vaultKey: CryptoKey, userId: string) {
  const rawClient = client as any;
  const existingState = rawClient.__ficonterDebtBoundaryState as BoundaryState | undefined;
  if (existingState) {
      ...
      get(target, property, receiver) {
        ...
        return (value: unknown, ...args: unknown[]) => {
          ...
          return deferredMutation(async (calls) => {
            const debtId = String(record.id ?? findEqValue(calls, "id") ?? crypto.randomUUID());
            ...
            const encryptedRecord = stripPrivateFields({
              ...record,
            ...
            let mutation = original.call(target, encryptedRecord, ...args);
            if (property === "update") mutation = mutation.eq("e2ee_revision", revision);
            const result = await replayBuilder(mutation, calls);
            return restoreMutationResult(result, record);
          });
        };
      },
    });
  ...
  rawClient.rpc = (fn: string, args?: Record<string, unknown>, options?: unknown) => {
    ...
    return (async () => {
      if (fn === "record_debt_payment_atomic") {
        ...
        if (row.debt_kind === "credit_card" || String(row.category ?? "").toLowerCase() === "credit card") {
          return originalRpc(fn, args, options);
        }
        ...
        const current = await decryptDebtPayload(state.vaultKey, state.userId, row);
        ...
        const newDebtPayload = await encryptDebtPayload(state.vaultKey, state.userId, debtId, updated);
        const paymentPayload = await encryptDebtPaymentPayload(state.vaultKey, state.userId, paymentId, {
          amount,
      ...
      if (debtRow.debt_kind === "credit_card" || String(debtRow.category ?? "").toLowerCase() === "credit card") {
        return originalRpc(fn, args, options);
      }
      ...
      const payment = await decryptDebtPaymentPayload(state.vaultKey, state.userId, paymentRow);
      const debt = await decryptDebtPayload(state.vaultKey, state.userId, debtRow);
      ...
      const restoredPayload = await encryptDebtPayload(state.vaultKey, state.userId, debtRow.id, restored);
