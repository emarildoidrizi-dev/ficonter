...
const checks = [
  ...
  ['Language is the deliberate immediate-confirm exception', languageSelector.includes('The interface changes immediately') && !languageSelector.includes('draftLanguage')],
  ...
  ['Language selection persists through the stable provider', languageSelector.includes('void changeLanguage(nextLanguage, true)') && languageProvider.includes('await supabase.auth.getUser()')],
  ['Language is applied immediately before account persistence', languageProvider.includes('applyCommittedLanguage();') && languageProvider.indexOf('applyCommittedLanguage();') < languageProvider.indexOf('const persistAccountLanguage = async')],
