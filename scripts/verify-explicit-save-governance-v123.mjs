...
const checks = [
  ...
  ['Language is the deliberate immediate-confirm exception', languageSelector.includes('choosing an option is itself the confirmation action') && !languageSelector.includes('draftLanguage')],
  ...
  ['Language applies immediately when selected', languageProvider.includes('applyCommittedLanguage(normalized);') && languageProvider.indexOf('applyCommittedLanguage(normalized);') < languageProvider.indexOf('await supabase.auth.getUser()')],
  ['Language persistence failure restores the previous confirmed language', languageProvider.includes('applyCommittedLanguage(previous);')],
