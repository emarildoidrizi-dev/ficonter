...
const encrypted = read("components/EncryptedFinancialIndependenceWorkspace.tsx");
...
expect(component.includes("get_financial_independence_inputs"), "Legacy aggregate RPC adapter remains available");
expect(component.includes("normalizeNetWorthGrowthInputs(payload.netWorthGrowth)") && component.includes("normalizeSavingsIntelligenceInputs(payload.savingsIntelligence)") && component.includes("normalizeEmergencyFundInputs(payload.emergencyFund)"), "Aggregate FI inputs are normalized before base-currency reconciliation");
...
expect(component.includes("subscribeFiconterDataChanges") && component.includes("isFinancialDataScope"), "Financial realtime refresh uses shared event governance");
expect(page.includes("EncryptedFinancialIndependenceWorkspace") && page.includes("userId={user.id}"), "Server page enters authenticated encrypted FI workspace");
expect(encrypted.includes("useVault") && encrypted.includes("useBaseCurrencySourceData(userId)"), "Encrypted FI workspace requires Vault and shared financial source");
expect(encrypted.includes("installFinancialIndependenceE2eeBoundary"), "Encrypted FI workspace installs the FI E2EE boundary");
