...
const enginePath = "lib/wealth/savingsIntelligence.ts", componentPath = "components/SavingsIntelligence.tsx", encryptedPath = "components/EncryptedSavingsWorkspace.tsx", cssPath = "components/SavingsIntelligence.module.css", pagePath = "app/dashboard/savings/page.tsx", sqlPath = "supabase/phase2_savings_intelligence.sql", sidebarPath = "components/Sidebar.tsx", packagePath = "package.json";
for (const file of [enginePath, componentPath, encryptedPath, cssPath, pagePath, sqlPath]) check(`exists: ${file}`, exists(file));
const engine = read(enginePath), periods = read("lib/wealth/averagePeriods.ts"), component = read(componentPath), encrypted = read(encryptedPath), page = read(pagePath), sql = read(sqlPath), sidebar = read(sidebarPath), packageJson = JSON.parse(read(packagePath));
...
check("server page enters encrypted Savings workspace", page.includes("EncryptedSavingsWorkspace") && page.includes("userId={user.id}"));
check("encrypted workspace builds Savings inputs from shared source", encrypted.includes("useBaseCurrencySourceData(userId)") && encrypted.includes("buildSavingsClientInputs(source)"));
check("encrypted workspace installs Savings E2EE boundary", encrypted.includes("installSavingsIntelligenceE2eeBoundary"));
