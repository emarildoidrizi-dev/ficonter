-- FICONTER V5 — Beta invitation security for NEW + EXISTING customer accounts.
...
--   * Existing accounts that are already Beta are preserved; this migration does not
--     mass-downgrade legitimate current testers.
...
begin
  ...
  -- Already Beta is not a transition, so no new invite consumption is necessary.
  if current_plan = 'beta' then
    return true;
  end if;
...
comment on function public.activate_ficonter_beta_for_existing_user(uuid, text) is
  'Existing customer Beta activation. Service-only RPC validates and consumes a private invitation before changing the account to Beta.';
