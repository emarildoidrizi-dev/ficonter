-- FICONTER · Emergency Recovery Envelope foundation
-- Stores only public-key-encrypted copies of the SAME Financial Vault key.
-- No readable Vault key or KMS/HSM private key is stored in Supabase.
...
-- Atomically installs a newly wrapped customer recovery envelope AND its emergency envelope.
-- This is used only after an authenticated customer has claimed a valid Recovery Access grant.
